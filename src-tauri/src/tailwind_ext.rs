...
/// Get embedded Tailwind v4 default utilities
fn get_default_utilities() -> Vec<TailwindCompletion> {
    // Tailwind v4 core utilities - embedded for offline use
    let utilities = vec![
        // Layout
...
pub fn tailwind_start_watcher(root: String, app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    std::thread::spawn(move || {
        ...
        let mut watcher: RecommendedWatcher = match Watcher::new(tx, config) {
            Ok(w) => w,
            Err(e) => {
                eprintln!("Failed to create Tailwind watcher: {}", e);
